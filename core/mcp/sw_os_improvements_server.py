#!/usr/bin/env python3
"""
MCP Server for sw_os Improvements Backlog System

Provides tools for capturing and managing sw_os system improvement ideas with:
...
BACKLOG_FILE = BASE_DIR / 'System' / 'sw_os_Backlog.md'
...
# Keywords that indicate a changelog entry is relevant to sw_os
    ...
    if HAS_QMD and is_qmd_available():
        try:
            results = vault_search(
                ...
                limit=5,
                min_score=0.3,
                fallback_glob="System/sw_os_Backlog.md"
            )
            for r in results:
...
def parse_backlog_file() -> List[Dict[str, Any]]:
    """Parse the sw_os backlog file and extract all ideas"""
    if not BACKLOG_FILE.exists():
        return []
...
def score_changelog_relevance(feature_text: str) -> int:
    """Score how relevant a changelog entry is to sw_os (0-100)"""
...
def initialize_backlog_file():
    """Create the sw_os backlog file with initial structure"""
    SYSTEM_DIR.mkdir(parents=True, exist_ok=True)
    
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M')
    
    content = f"""# sw_os System Improvement Backlog

*Last ranked: {timestamp}*

Welcome to your sw_os system improvement backlog! This file tracks ideas for making sw_os better.
...
"""
    ...
    logger.info(f"Created sw_os backlog file at {BACKLOG_FILE}")
...
def mark_idea_implemented(idea_id: str, implementation_date: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not BACKLOG_FILE.exists():
        return {
            'success': False,
            'error': 'sw_os backlog file does not exist'
        }
...
async def handle_list_tools() -> list[types.Tool]:
    ...
    return [
        types.Tool(
            name="capture_idea",
            description="Capture a new sw_os system improvement idea. Always available for quick capture from any context.",
            inputSchema={
                "type": "object",
                "properties": {
                    "title": {
        ...
        types.Tool(
            ...
            description="Scan Anthropic Claude Code changelog for new features relevant to sw_os. Creates new backlog ideas or enriches existing ones with 'Why Now?' evidence. Call during /dex-whats-new or /daily-plan.",
            inputSchema={
                "type": "object",
                "properties": {
                    "days_back": {
...
async def _handle_call_tool_inner(
    ...
    if name == "capture_idea":
        ...
        # Generate ID and add to sw_os backlog
        ...
        if success:
            ...
            result = {
                "success": False,
                "error": "Failed to add idea to sw_os backlog"
            }
        ...
        for entry in relevant_entries:
            ...
            if best_match and best_similarity > 0.4:
                ...
                description = (
                    ...
                    f"Evaluate how sw_os could leverage this for user workflows."
                )
        ...
        result = {
            ...
            "message": f"Scanned {len(entries)} changelog entries. {len(relevant_entries)} were relevant to sw_os. Created {ideas_created} new ideas, enriched {ideas_enriched} existing ideas."
        }
        return [types.TextContent(type="text", text=json.dumps(result, indent=2, cls=DateTimeEncoder))]
...
async def _main():
    ...
    logger.info(f"Starting sw_os Improvements MCP Server")
