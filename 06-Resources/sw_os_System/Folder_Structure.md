# sw_os Folder Structure (PARA Method)

sw_os uses the PARA method for organization: **Projects**, **Areas**, **Resources**, and **Archives**.
...
sw_os creates this structure during onboarding (adapted to your role):
...
Most users won't edit this directly—sw_os manages it. But when you want to adjust strategic direction or preferences, the key files are here.

**Background automation:** sw_os also includes scripts in `.scripts/` that run automatically:
...
New to sw_os? Run `/setup` or just ask to start onboarding. The structure will be customized for your role and working style.
...
Want to explore sw_os with sample data before adding your own? Run `/dex-demo on` to work with pre-populated demo content for "Alex Chen," a fictional PM at TechCorp. Demo mode creates a sandboxed `System/Demo/` folder so your real vault stays untouched.
...
- `06-Resources/sw_os_System/Dex_Jobs_to_Be_Done.md` — Why the system exists
- `06-Resources/sw_os_System/Dex_System_Guide.md` — How to use everything
