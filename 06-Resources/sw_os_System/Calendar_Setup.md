# Connect Google Calendar to sw_os (Mac)

This guide is for **Mac users** who use **Google Calendar** and want sw_os to show their real meetings—including recurring ones like weekly 1:1s—when they run `/daily-plan` or ask "what's on my calendar today?"
...
sw_os reads your calendar from the **Calendar app** that came with your Mac. So you add your Google account to that app once, let Cursor use it, and sw_os sees your meetings.
...
sw_os doesn't talk to Google directly. It uses the built-in **Calendar** app on your Mac. So first we get your Google calendar into that app.
...
Your Google events will now appear in the Calendar app. Once they're here, sw_os can see them too.
...
4. Click **Cursor** and set access to **Full** (not "Add Only") so sw_os can read your events.
...
| **No meetings or wrong dates for recurring events** | Make sure you did both steps above. If you installed sw_os without running the installer (e.g. you installed Python packages yourself), open Terminal and run: `pip3 install --user pyobjc-framework-EventKit`, then restart Cursor. |
...
## Optional: Tell sw_os which calendar is "work"

If you have several calendars and want sw_os to focus on one (e.g. your work calendar) for faster answers, you can set it in **System/user-profile.yaml** under a `calendar` section with `work_calendar: "your.email@company.com"` (use the exact name as it appears in the Calendar app). You can skip this—sw_os will still show your events without it.
...
After that, your Google Calendar meetings show up in sw_os on the right days, including recurring events.
