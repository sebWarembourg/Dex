...
**Personal Reference** — Full documentation for your sw_os knowledge system.
...
After mtgs → sw_os extracts action items and updates person pages
...
Demo mode lets you explore sw_os with pre-populated sample content without affecting your real vault.
...
- sw_os Backlog with 10 ranked improvement ideas
...
sw_os includes a Python MCP server (`core/mcp/work_server.py`) providing deterministic work management operations across the complete planning hierarchy.
...
4 templates in `System/Templates/` used by sw_os automation:
...
These templates are automatically applied when sw_os creates files through skills. You can modify them to match your preferences.
...
**What sw_os Does:** Helps you capture evidence of your work throughout the year, so when opportunity knocks, you're ready.

### Getting Started (`/career-setup`)

Run `/career-setup` once to set up your career folder. sw_os will ask you to share:
- Your current job description
- Your company's career ladder (the document showing what's expected at each level)
- Your latest performance review
- Your long-term career goals

This creates a `Career/` folder where sw_os will track your growth over time.

### Your Career Coach (`/career-coach`)

Think of this as having a career coach on call. Four ways to use it:

**Weekly Check-in** — Every Friday, reflect on the week. What went well? What was hard? What did you learn? sw_os helps you spot patterns over time.

**Monthly Deep Dive** — Once a month, step back. Are you developing the skills you need? Making progress toward your goals? sw_os compares your recent work against your career ladder to show where you're strong and where you need more evidence.

**Before Your Review** — Run this 1-2 weeks before your performance review. sw_os compiles everything you've accomplished and maps it to what your company values. You walk into the review with concrete examples ready.

**Promotion Assessment** — Thinking about the next level? sw_os analyzes your evidence and tells you honestly: "You're ready" or "Here's what you need to demonstrate first." It looks at four things:
...
1. sw_os walks through your work history, one role at a time
...
You can pause and come back later - sw_os remembers where you left off.

**Bonus:** sw_os also generates your LinkedIn profile (headline and about section) with proper character limits enforced.
...
Later, during `/week-review`, sw_os notices: "You completed 'Ship payments redesign' this week. Want to capture this as career evidence?"
...
With sw_os, career development becomes a habit, not a crisis. Every week you capture a little bit. By the time you need it, you have a complete picture of your growth.

---

## Improving sw_os Itself
...
**What sw_os Does:** Captures your improvement ideas as you think of them, then helps you prioritize which ones matter most.

### Capturing Ideas

Anytime you think "sw_os should do X differently," just mention it. sw_os will:
...
When you're ready to work on improvements, run `/dex-backlog`. sw_os analyzes all your ideas and tells you which ones would help you most, based on:

- **How much time it would save you daily** (most important)
- **Whether it fits how you actually use sw_os** (no point building features you won't use)
- **Whether it makes the system faster** (nobody wants a slow system)
- **Whether it helps sw_os learn and remember better**
- **Whether it makes sw_os more proactive**
...
Pick an idea and run `/dex-improve [idea]`. sw_os becomes your design partner:
...
sw_os treats itself as a product you're continuously improving. Ideas compound, the system adapts, and it gets better the longer you use it.

---

## How sw_os Learns and Improves Itself
...
**What sw_os Does:** Automatically monitors for improvements and reminds you when there's something new worth knowing about.

### Background Monitoring

Behind the scenes, sw_os runs two quiet background checks:

**Checking for Claude Updates (every 6 hours)**  
sw_os checks if Anthropic has released new Claude Code features. When it finds something new, you'll see a heads-up next time you start working: "🆕 New Claude Code features detected! Run `/dex-whats-new` to review."

**Learning Review Prompts (daily at 5pm)**  
As you work, sw_os captures learnings in `System/Session_Learnings/`. When you accumulate 5+ learnings that haven't been reviewed yet, sw_os reminds you: "📚 You have 7 pending learnings from this week. Worth reviewing?"

Both happen automatically with no action from you. The system stays current on its own.

### Learning Files

sw_os maintains two special files that get smarter over time:

**`06-Resources/Learnings/Mistake_Patterns.md`**  
When you make a mistake and fix it, sw_os can save that pattern. Next time you face a similar situation, it reminds you what went wrong before.

**Example:** You accidentally broke something by editing the wrong file. sw_os logs: "Before editing system files, always check if there's a template version." Later, when you go to edit a system file, sw_os surfaces that reminder.

**`06-Resources/Learnings/Working_Preferences.md`**  
sw_os learns how you like to work and applies those preferences consistently.

**Example:** You mention "I prefer bullet points over paragraphs in summaries." sw_os saves that preference. From then on, all summaries use bullet points without you having to ask.
...
With automation, sw_os nudges you at the right time. The system improves itself through quiet background work, and only asks for your input when it matters.

---

## How sw_os Remembers Context
...
**What sw_os Does:** Automatically loads relevant context in the background so you don't have to go hunting for it.

### Smart Context Loading

When you're reading any file that mentions people or companies, sw_os quietly:
...
**Example:** You open a meeting note from a customer call. The note mentions "Acme Corp." sw_os automatically loads the Acme Corp company page, sees you've had 5 meetings with them in the past month, notices there are 3 open action items, and uses that context to help you prepare better.

### Discovering Unused Features (`/dex-level-up`)

sw_os quietly keeps track of which features you've used:
...
When you run `/dex-level-up`, sw_os shows you relevant features you haven't tried yet. It's personalized based on your role and what you're already using.
...
**The Problem:** You tell sw_os about a risk on Monday, and by Wednesday it's forgotten. Each session starts from scratch, so recurring patterns — stalled deals, people dropping off radar, overloaded weeks — never get flagged consistently.

**What sw_os Does:** Six operational agents now persist memory across sessions. They remember what they've flagged, what you've resolved, and escalate patterns that keep coming up.
...
Without memory, every planning session starts from zero. The same blind spots persist because nothing tracks them across sessions. With agent memory, sw_os builds a picture over time and catches what you'd otherwise miss.

**Reference:** See `06-Resources/sw_os_System/Memory_Ownership.md` for details on how agent memory works.
...
**What sw_os Does:** Heavy skills now run in isolated context. Your main conversation stays clean all day.
...
**Reference:** See `06-Resources/sw_os_System/Named_Sessions_Guide.md` for details on how named sessions work.
...
After processing meetings, sw_os automatically updates person pages with meeting references. You no longer need to manually link meetings to the people who attended.
...
After creating your daily plan, sw_os generates a condensed quickref — a one-glance summary of your day.
...
**Reference:** See `06-Resources/sw_os_System/Background_Processing_Guide.md` for details on how automated follow-ups work.

---

## Vault Maintenance

Ask sw_os to "run vault maintenance" at any time for a health check on your vault.
...
Just say "run vault maintenance" in any conversation. sw_os will scan and report what it finds, then suggest actions for each issue:
...
During onboarding, sw_os sets up the core features everyone needs. But depending on your role, there are additional specialized skills available:
...
3. sw_os moves them from `_available/` to active `.claude/skills/`
...
- `/reset` — Restructure sw_os based on role change
...
**Where sw_os stores what it learns about how you work.**
...
**During Your Day:**
- sw_os captures learnings in `System/Session_Learnings/` as you work
- Each entry notes what happened, why it matters, and what to fix

**During `/daily-review`:**
- sw_os asks: "This sounds like a mistake pattern. Save it for next time?"
- Or: "This seems like a workflow preference. Add to Working_Preferences.md?"

**During `/week-review`:**
- sw_os scans the week's session learnings for patterns
- Suggests consolidating repeated lessons into Mistake_Patterns or Working_Preferences

**Automatic Prompts:**
- Daily at 5pm, sw_os checks if you have 5+ pending learnings
...
sw_os remembers for you. Every session builds on past lessons, and the system gets smarter the longer you use it.

For more on how sw_os learns and improves itself, see the **"How sw_os Learns and Improves Itself"** section above.
...
sw_os includes built-in MCP servers and can work with additional integrations.

### Built-in MCP Servers

sw_os includes seven custom MCP servers in `core/mcp/`:
...
| **sw_os Improvements** | `dex_improvements_server.py` | Improvement idea capture, duplicate detection, backlog management |
| **Update Checker** | `update_checker.py` | Automatic sw_os update detection from GitHub |
...
sw_os processes meetings from Granola to extract structured insights, action items, and update person pages. Choose between manual and automatic processing.
...
5. **Integrate** — Update CLAUDE.md and this guide so sw_os knows how to use it
...
sw_os leverages these Claude Code capabilities. For deeper understanding:
...
- `06-Resources/sw_os_System/Dex_Jobs_to_Be_Done.md` — Why the system exists (conceptual)
- `06-Resources/sw_os_System/Memory_Ownership.md` — How agent memory works across sessions
- `06-Resources/sw_os_System/Named_Sessions_Guide.md` — Context isolation and named sessions
- `06-Resources/sw_os_System/Background_Processing_Guide.md` — Automated follow-ups and background tasks
...
*This guide covers how to use sw_os. For why it exists, see the Jobs to Be Done document.*
