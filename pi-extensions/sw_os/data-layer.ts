/**
 * Data Layer for sw_os Extension
 * 
...
function writeFileSync(filePath: string, content: string): boolean {
  try {
    ...
    console.error(`[sw_os Data Layer] Failed to write file: ${filePath}`, error);
    return false;
  }
}
...
export function loadTasks(): Task[] {
  console.log("[sw_os Data Layer] Loading tasks from:", TASKS_PATH);
  const content = readFileSync(TASKS_PATH);
  if (!content) {
    console.log("[sw_os Data Layer] ❌ Tasks file not found or empty");
    return [];
  }

  console.log("[sw_os Data Layer] Tasks file read, length:", content.length);
  const tasks: Task[] = [];
  ...
  for (const line of content.split("\n")) {
    ...
    if (sectionMatch) {
      ...
      console.log(`[sw_os Data Layer] Section found: ${section} -> Priority: ${currentPriority}`);
      continue;
    }
    ...
    if (task) {
      ...
      console.log(`[sw_os Data Layer] Task parsed: [${task.priority}] ${task.title.substring(0, 40)}`);
      tasks.push(task);
    }
  }

  console.log(`[sw_os Data Layer] ✅ Loaded ${tasks.length} total tasks`);
  return tasks;
}
...
export function loadWeekPriorities(): WeekPriority[] {
  ...
  console.log("[sw_os Data Layer] Loading week priorities from:", weekFilePath);
  
  const content = readFileSync(weekFilePath);
  if (!content) {
    console.log("[sw_os Data Layer] ❌ Week priorities file not found or empty");
    return [];
  }

  console.log("[sw_os Data Layer] Week priorities file read, length:", content.length);

  // Look for "## 🎯 Top 3 This Week" section
  const topThreeMatch = content.match(/## 🎯 Top 3 This Week[\s\S]*?\n([\s\S]*?)(?=\n---|\n##|$)/);
  if (!topThreeMatch) {
    console.log("[sw_os Data Layer] ❌ Could not find '## 🎯 Top 3 This Week' section");
    return [];
  }

  const section = topThreeMatch[1]!;
  console.log("[sw_os Data Layer] Found Top 3 section, length:", section.length);
  ...
  while ((match = goalRegex.exec(section)) !== null) {
    ...
    console.log(`[sw_os Data Layer] Goal found: ${goalText.substring(0, 50)}`);
  ...
  console.log(`[sw_os Data Layer] ✅ Loaded ${priorities.length} priorities`);
  return priorities;
}
