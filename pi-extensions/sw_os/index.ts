/**
 * sw_os for PI - The Complete Extension
 * 
 * Transforms sw_os into a proactive, parallel, intelligent productivity system.
 * 
 * Superpowers:
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    if (ctx.hasUI) {
      ...
      try {
        ...
        ctx.ui.setWidget("dex-dashboard", (_tui, _theme) => ({
          render: (width) => {
            try {
              ...
              console.error("[sw_os] Dashboard render error:", error);
              return [];
            }
          },
          invalidate: () => {},
        ...
        const footerParts = ["● sw_os"];
        if (taskSummary.p0Count > 0) {
          footerParts.push(`${taskSummary.p0Count} P0`);
        }
        ctx.ui.setStatus("sw_os", footerParts.join(" | "));
        
      } catch (error) {
        console.error("[sw_os] Dashboard error:", error);
        // Fallback to simple status
        const taskSummary = getTaskSummary();
        ctx.ui.setStatus("sw_os", `● sw_os | ${taskSummary.totalOpen} tasks`);
      }
    }
  ...
  pi.on("session_shutdown", async (_event, ctx) => {
    ...
    if (fs.existsSync(path.join(VAULT_PATH, ".git"))) {
      try {
        await execAsync(`cd "${VAULT_PATH}" && git add -A && git commit -m "sw_os auto-sync: ${formatTimestamp()}" && git push 2>/dev/null`);
      } catch {}
    }
  });
  ...
  pi.registerTool({
    ...
    label: "Vault Search",
    description: "Search across the sw_os vault for content, people, projects, or tasks. Returns relevant excerpts with file paths.",
  ...
  pi.registerTool({
    name: "pi_dex_status",
    label: "sw_os Status",
    description: "Get a quick overview of your sw_os state: tasks, meetings, week progress.",
    parameters: Type.Object({}),
    
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      const status = await getDexStatus();
      
      let output = "## sw_os Status\n\n";
  ...
  pi.registerCommand("status", {
    description: "Show sw_os status (tasks, meetings, week progress)",
  ...
  pi.registerCommand("sw_os", {
    description: "sw_os help and status",
    handler: async (args, ctx) => {
      const status = await getDexStatus();
      ctx.ui.notify(
        `sw_os for PI\n` +
  ...
  pi.registerCommand("refresh", {
    ...
    handler: async (args, ctx) => {
      ...
      try {
        ...
        console.error("[sw_os] Refresh error:", error);
