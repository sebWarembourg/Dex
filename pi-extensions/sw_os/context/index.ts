 ...
 * Orchestrates all context injection for sw_os
 * 
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    try {
      ...
      if (contexts.length > 0) {
        const fullContext = [
          "<dex_context>",
          "<!-- Automatically injected context from sw_os vault -->",
          "",
          contexts.join("\n\n"),
          "",
          "</dex_context>",
        ...
        if (sources.length > 0) {
          console.error(`[sw_os] Injected context: ${sources.join(", ")}`);
        }
      ...
      console.error(`[sw_os] Context injection error: ${e}`);
    }
  ...
  pi.on("tool_result", async (event, ctx) => {
    ...
    try {
      ...
      if (contexts.length > 0) {
        ...
        console.error(`[sw_os] Reactive context: ${sources.join(", ")}`);
      }
    } catch (e) {
      console.error(`[sw_os] Reactive context error: ${e}`);
    }
